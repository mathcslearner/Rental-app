...
const Applications = () => {
    ...
    const {data: applications, isLoading, isError} = useGetApplicationsQuery({userId: authUser?.cognitoInfo?.userId, userType: "manager"}, {skip: !authUser?.cognitoInfo?.userId});
    ...
    const filteredApplication = applications?.filter((application) => {
        ...
        return application.status.lowerCase() === activeTab;
    })
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full my-5">
                ...
                {["all", "pending", "approved", "denied"].map((tab) => (
                    <TabsContent key={tab} value={tab} className="mt-5 w-full">
                        {filteredApplication.filter((application) => tab === "all" || application.status.toLowerCase() === tab).map(
                            (application) => (
